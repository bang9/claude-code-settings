...
usage() {
  cat <<'EOF'
Usage: pr-reviewer/digest/scripts/run.sh --input <raw-input.json> [options]

Options:
  --input <path>              Raw build input JSON for pr-reviewer digest
  --timeout <duration>        Prompt timeout (default: 0s, disabled)
  --entry <path>              Prompt entry module (default: pr-reviewer digest web/app.mjs)
...
if [[ -z "$NODE_BIN" ]]; then
  echo "node is required to run pr-reviewer digest" >&2
  exit 1
fi
